...
_DRAFT_PROMPT = """你是一个专业研究员，请**严格基于以下研究资料**撰写一份完整的研究报告。
...
- 结构：摘要 / 背景 / 主体（分节）/ 结论
- 字数 1500-3000 字
- **每一个事实性陈述后面，必须用括号标注来源 URL 或文件名**（例如：来源: https://... 或 来源: 02_grpo_internal_benchmark.md）
- **资料中没有提到的内容，一律不得编造**。如果资料不足以回答某个子问题，必须在报告中明确说明"该子问题缺乏足够资料"
- **本地知识库(local_knowledge_search)返回的内容优先级最高**——如果它与你的预训练知识冲突，以本地资料为准
- 输出 Markdown 格式
"""
